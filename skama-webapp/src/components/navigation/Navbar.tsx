...
const navLinks = [
  ...
  { label: 'Favoritos', path: ROUTES.wishlist, match: [ROUTES.wishlist] },
  { label: 'Orden', path: ROUTES.checkout, match: [ROUTES.checkout, ROUTES.cart] },
];
  ...
  const mobileNav = (
    <div className={`sk-drawer${mobileOpen ? ' is-open' : ''}`} aria-hidden={!mobileOpen}>
      <div className="sk-drawer__panel">
        ...
        <nav className="sk-drawer-nav" aria-label="Navegación móvil">
          {navLinks.map((link) => (
            <RouterLink
              key={link.path}
              to={link.path}
  ...
  return (
    <>
      <header className={`sk-navbar${isScrolled ? ' is-scrolled' : ''}`} data-navbar>
        ...
        <nav className="sk-navbar__links" aria-label="Navegación principal">
          {navLinks.map((link) => (
            <RouterLink
              key={link.path}
              to={link.path}
