/*
  SKAMA - Aplicar cambios pendientes (18-ago-2026) en UNA sola corrida.

  Seguro de reejecutar: IF EXISTS / IF NULL + CREATE OR ALTER.

  Orden interno (no cambiar):
    1. Constraints (LOW_STOCK_ALERT, review unica por pedido)
    2. usp_EmailNotification_Insert
    3. RF-008 / RF-010: promociones, carrito y checkout
    4. MainImageUrl en usp_Product_GetAll / GetById  <-- DEBE ir al final

  NO ejecutes despues 20260818_fix_ui_rf004_rf017.sql completo:
  pisa GetAll y CreateFromCart y quita descuentos / imagen principal.

  Uso (SSMS o Azure Data Studio):
    - Conectate a la misma base que usa la API (Azure SQL o local)
    - Abre este archivo y ejecuta todo (F5)
*/
SET NOCOUNT ON;
GO
...
/* -------------------------------------------------------------------------- */
/* 4. MainImageUrl (version final de GetAll / GetById)                        */
/* -------------------------------------------------------------------------- */

/*
  SKAMA - Imagen principal en el listado de productos.

  Evita N+1 de GET /api/products/{id}/images en el catalogo.
  Ejecutar en bases existentes para actualizar stored procedures.
*/
GO
