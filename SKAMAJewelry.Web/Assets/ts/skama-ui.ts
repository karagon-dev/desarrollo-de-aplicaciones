(() => {
    ...
    type CatalogCartItem = CatalogProductData & {
        quantity: number;
    };

    const root = document.documentElement;
    const body = document.body;
    const dateFormatRevealTimers = new WeakMap<FieldControl, number>();
    ...
    document.querySelectorAll<HTMLElement>("[data-icon]").forEach((icon) => {
        const svg = iconMap[icon.dataset.icon as string];
        if (svg) {
            icon.innerHTML = svg;
        }
    });

    const updateThemeToggleState = (theme: string): void => {
    ...
    const isDateFieldControl = (control: FieldControl): control is HTMLInputElement => {
        return control instanceof HTMLInputElement && control.type === "date";
    };

    const parseTimeInMs = (value: string): number => {
    ...
    const getLabelRiseDuration = (field: Element): number => {
        const labelLetters = Array.from(field.querySelectorAll<HTMLElement>(".sk-field__label span"));
    ...
    const updateDateFormatVisibility = (control: FieldControl, field: Element, shouldFloat: boolean): void => {
    ...
    const updateFloatingFieldState = (control: FieldControl): void => {
    ...
    const initializeFloatingFields = (): void => {
        document.querySelectorAll<HTMLElement>(".sk-field__label").forEach((label) => {
        ...
        document.querySelectorAll<FieldControl>(".sk-field input, .sk-field textarea, .sk-field select").forEach((control) => {
    ...
    let activeDrawerTrigger: HTMLElement | null = null;
    let activeCatalogProduct: CatalogProductData | null = null;
    let activeCatalogAddButton: HTMLButtonElement | null = null;
    ...
    const closeDrawer = (drawer: Element | null): void => {
        drawer?.classList.remove("is-open");
        drawer?.setAttribute("aria-hidden", "true");
        body.classList.remove("is-scroll-locked");
        document.querySelectorAll<HTMLElement>("[data-drawer-open]").forEach((button) => {
    ...
    const getCatalogProductData = (card: HTMLElement): CatalogProductData => {
    ...
    const formatCatalogPrice = (value: number): string => `CRC ${priceFormatter.format(value)}`;

    const getProductQuantityLimit = (product: CatalogProductData): number => {
    ...
    const formatStockLabel = (product: CatalogProductData): string => {
        const units = product.stock === 1 ? "unidad" : "unidades";
        const limit = getProductQuantityLimit(product);

        return product.isLimited
            ? `Disponible: ${product.stock} ${units}. Límite de compra: 1 joya limitada por persona.`
            : `Disponible: ${product.stock} ${units}. Máximo por pedido: ${limit}.`;
    };

    const normalizeCatalogProductData = (value: unknown): CatalogProductData | null => {
        if (!isRecord(value)) {
            return null;
        }
    ...
    const showDialog = (dialog: HTMLDialogElement | null): void => {
    ...
    const openLimitedAccessDialog = (): void => {
        const detailModal = document.querySelector<HTMLDialogElement>("[data-product-detail-modal]");
        const limitedModal = document.querySelector<HTMLDialogElement>("[data-limited-access-modal]");
    ...
    const syncLimitedPurchaseButtons = (): void => {
        const authenticated = isUserAuthenticated();

        document.querySelectorAll<HTMLButtonElement>("[data-limited-purchase-trigger]").forEach((button) => {
    ...
    const setLimitedCarouselSlide = (carousel: HTMLElement, index: number): void => {
        const slides = Array.from(carousel.querySelectorAll<HTMLElement>("[data-limited-slide]"));
        const dots = Array.from(carousel.querySelectorAll<HTMLButtonElement>("[data-limited-carousel-dot]"));
    ...
    const initializeLimitedCarousels = (): void => {
        document.querySelectorAll<HTMLElement>("[data-limited-carousel]").forEach((carousel) => {
    ...
    function getLimitedCartQuantity(): number {
        return getStoredCartItems()
            .filter((item) => item.isLimited)
            .reduce((total, item) => total + item.quantity, 0);
    }

    function getRemainingProductQuantity(product: CatalogProductData): number {
    ...
    const openCatalogAddDialog = (product: CatalogProductData, sourceButton: HTMLButtonElement | null): void => {
        ...
        const detailModal = document.querySelector<HTMLDialogElement>("[data-product-detail-modal]");
        const addModal = document.querySelector<HTMLDialogElement>("[data-product-add-modal]");
        const quantity = addModal?.querySelector<HTMLInputElement>("[data-product-add-quantity]");
        const submitButton = addModal?.querySelector<HTMLButtonElement>("button[type='submit']");
        ...
        setText(
            addModal?.querySelector("[data-product-add-stock]") || null,
            remainingQuantity > 0
                ? formatStockLabel(product)
                : product.isLimited
                    ? "Ya alcanzaste el límite de 1 joya limitada por persona."
                    : "Ya agregaste la cantidad máxima disponible para esta joya."
        );
        ...
        if (submitButton) {
            ...
            submitButton.textContent = remainingQuantity === 0 ? "Límite alcanzado" : "Agregar selección";
        }
    ...
    const openCatalogProductDetail = (card: HTMLElement): void => {
        const product = getCatalogProductData(card);
        activeCatalogProduct = product;
        activeCatalogAddButton = card.querySelector<HTMLButtonElement>("[data-product-add-trigger]");

        const modal = document.querySelector<HTMLDialogElement>("[data-product-detail-modal]");
        const image = modal?.querySelector<HTMLImageElement>("[data-product-detail-image]");
        const addButton = modal?.querySelector<HTMLButtonElement>("[data-product-detail-add]");
    ...
    const normalizeCatalogCartItem = (value: unknown): CatalogCartItem | null => {
        if (!isRecord(value)) {
            return null;
        }
    ...
    const getStoredCartItems = (): CatalogCartItem[] => {
        try {
            const parsed = JSON.parse(localStorage.getItem(cartStorageKey) || "[]");
            return Array.isArray(parsed)
                ? parsed.map(normalizeCatalogCartItem).filter((item): item is CatalogCartItem => item !== null)
                : [];
        } catch {
            return [];
        }
    ...
    const storeCartItems = (items: CatalogCartItem[]): void => {
        localStorage.setItem(cartStorageKey, JSON.stringify(items.filter((item) => item.quantity > 0)));
    };

    const getCartItemCount = (items = getStoredCartItems()): number => {
        return items.reduce((total, item) => total + item.quantity, 0);
    };

    const getCartSubtotal = (items = getStoredCartItems()): number => {
        return items.reduce((total, item) => total + item.price * item.quantity, 0);
    };

    const updateCartCounter = (items = getStoredCartItems()): void => {
        const count = getCartItemCount(items);
        document.querySelectorAll<HTMLElement>("[data-cart-count]").forEach((counter) => {
            counter.textContent = "";
    ...
    const createCartDrawerItem = (item: CatalogCartItem): HTMLElement => {
    ...
    const renderCartDrawer = (): void => {
        ...
        document.querySelectorAll<HTMLElement>("[data-cart-section]").forEach((section) => {
    ...
    const storeCatalogCartItem = (product: CatalogProductData, quantity: number): number => {
        const items = getStoredCartItems();
    ...
    const updateCartItemQuantity = (name: string, quantity: number): void => {
    ...
    const getStoredFavoriteItems = (): CatalogProductData[] => {
        try {
            const parsed = JSON.parse(localStorage.getItem(favoriteStorageKey) || "[]");
            return Array.isArray(parsed)
                ? parsed.map(normalizeCatalogProductData).filter((item): item is CatalogProductData => item !== null)
                : [];
        } catch {
            return [];
        }
    ...
    const storeFavoriteItems = (items: CatalogProductData[]): void => {
        localStorage.setItem(favoriteStorageKey, JSON.stringify(items));
    };

    const isFavoriteProduct = (product: CatalogProductData, favorites = getStoredFavoriteItems()): boolean => {
        return favorites.some((item) => item.id === product.id);
    };

    const updateFavoriteCounter = (): void => {
        const count = getStoredFavoriteItems().length;
        document.querySelectorAll<HTMLElement>("[data-favorite-count]").forEach((counter) => {
            counter.textContent = "";
            counter.hidden = count === 0;
    ...
    const syncFavoriteToggles = (): void => {
        const favorites = getStoredFavoriteItems();

        document.querySelectorAll<HTMLElement>("[data-favorite-toggle]").forEach((toggle) => {
            const productCard = toggle.closest<HTMLElement>("[data-product-card]");
            if (!productCard) {
                return;
            }
    ...
    const createFavoriteProductCard = (product: CatalogProductData): HTMLElement => {
        const card = document.createElement("article");
    ...
    const renderFavoritesPage = (): void => {
    ...
    const toggleFavoriteProduct = (product: CatalogProductData): void => {
    ...
    const getCarouselStep = (carousel: Element): number => {
    ...
    const updateCarouselPagination = (carousel: Element): void => {
        const track = carousel.querySelector<HTMLElement>("[data-carousel-track]");
        ...
        carousel.querySelectorAll<HTMLElement>("[data-carousel-page]").forEach((button, buttonIndex) => {
    ...
    document.addEventListener("click", (event) => {
        ...
        const adminShell = document.querySelector<HTMLElement>("[data-admin-shell] .admin-shell");
        const adminSidebar = document.querySelector<HTMLElement>("[data-admin-sidebar]");
        ...
        const adminSidebarToggle = target.closest<HTMLElement>("[data-admin-sidebar-toggle]");
        ...
        const adminSidebarOpen = target.closest<HTMLElement>("[data-admin-sidebar-open]");
        ...
        const adminPopoverTrigger = target.closest<HTMLElement>("[data-admin-popover-trigger]");
        if (adminPopoverTrigger) {
            const popover = adminPopoverTrigger.closest<HTMLElement>(".admin-popover");
            const isOpen = popover?.classList.contains("is-open") === true;

            document.querySelectorAll<HTMLElement>(".admin-popover.is-open").forEach((openPopover) => {
        ...
        const favoriteToggle = target.closest<HTMLElement>("[data-favorite-toggle]");
        if (favoriteToggle) {
            const productCard = favoriteToggle.closest<HTMLElement>("[data-product-card]");
            if (productCard) {
        ...
        const productDetailTrigger = target.closest<HTMLElement>("[data-product-detail-trigger]");
        if (productDetailTrigger) {
            const productCard = productDetailTrigger.closest<HTMLElement>("[data-product-card]");
            if (productCard) {
                openCatalogProductDetail(productCard);
            }
        }

        const carouselDot = target.closest<HTMLButtonElement>("[data-limited-carousel-dot]");
        if (carouselDot) {
            const carousel = carouselDot.closest<HTMLElement>("[data-limited-carousel]");
            if (carousel) {
                setLimitedCarouselSlide(carousel, Number(carouselDot.dataset.limitedCarouselDot || 0));
            }
        }

        const limitedPurchaseTrigger = target.closest<HTMLButtonElement>("[data-limited-purchase-trigger]");
        if (limitedPurchaseTrigger) {
            ...
            const productCard = limitedPurchaseTrigger.closest<HTMLElement>("[data-product-card]");
            if (productCard) {
                openCatalogAddDialog(getCatalogProductData(productCard), limitedPurchaseTrigger);
            }
        }

        const productAddTrigger = target.closest<HTMLButtonElement>("[data-product-add-trigger]");
        if (productAddTrigger) {
            const productCard = productAddTrigger.closest<HTMLElement>("[data-product-card]");
            if (productCard) {
                openCatalogAddDialog(getCatalogProductData(productCard), productAddTrigger);
            }
        }

        const productDetailAdd = target.closest<HTMLButtonElement>("[data-product-detail-add]");
        if (productDetailAdd && activeCatalogProduct) {
            openCatalogAddDialog(activeCatalogProduct, activeCatalogAddButton);
        }

        const optionChoice = target.closest<HTMLElement>(".product-option__choice");
        if (optionChoice) {
        ...
        if (galleryThumb) {
            const gallery = galleryThumb.closest<HTMLElement>(".product-gallery");
            const mainImage = gallery?.querySelector<HTMLImageElement>("[data-gallery-main]");
            const thumbImage = galleryThumb.querySelector<HTMLImageElement>("img");
        ...
        if (passwordToggle) {
            const field = passwordToggle.closest<HTMLElement>("[data-password-field], .sk-password-field");
            const input = field?.querySelector<HTMLInputElement>("[data-password-input]");
            const icon = passwordToggle.querySelector<HTMLElement>("[data-icon]");
            ...
            if (icon) {
                ...
                icon.innerHTML = iconMap[icon.dataset.icon as string] || "";
            }
        }

        const drawerOpen = target.closest<HTMLElement>("[data-drawer-open]");
        if (drawerOpen) {
            event.preventDefault();
            const drawer = document.getElementById(drawerOpen.dataset.drawerOpen as string);
        ...
        const openDrawer = target.closest<HTMLElement>("[data-drawer].is-open");
        ...
        if (modalOpen) {
            (document.getElementById(modalOpen.dataset.modalOpen as string) as HTMLDialogElement | null)?.showModal();
        }

        if (target.closest("[data-modal-close]")) {
            (target.closest("[data-modal]") as HTMLDialogElement | null)?.close();
        }

        const quantity = target.closest<HTMLElement>("[data-quantity-selector]");
        if (quantity) {
            const input = quantity.querySelector("[data-quantity-input]") as HTMLInputElement;
            const min = Number(input.min || 1);
            const max = Number(input.max || 99);
            const current = Number(input.value || min);

            if (target.closest("[data-quantity-decrease]")) {
                (input as unknown as { value: number | string }).value = Math.max(min, current - 1);
            }

            if (target.closest("[data-quantity-increase]")) {
                (input as unknown as { value: number | string }).value = Math.min(max, current + 1);
            }
        ...
        if (carousel) {
            const track = carousel.querySelector<HTMLElement>("[data-carousel-track]");
            const page = target.closest<HTMLElement>("[data-carousel-page]");
    ...
    document.addEventListener("change", (event) => {
        const input = (event.target as Element).closest<HTMLInputElement>("[data-cart-item-quantity]");
        if (!input) {
            return;
        }
    ...
    document.querySelector<HTMLFormElement>("[data-product-add-form]")?.addEventListener("submit", (event) => {
        event.preventDefault();

        const form = event.currentTarget as HTMLFormElement;
        const modal = form.closest<HTMLDialogElement>("[data-product-add-modal]");
        const quantityInput = form.querySelector<HTMLInputElement>("[data-product-add-quantity]");
        ...
        if (activeCatalogProduct) {
            ...
            if (activeCatalogAddButton) {
                activeCatalogAddButton.textContent = addedQuantity > 0 ? `Agregado x${addedQuantity}` : "Límite alcanzado";
                activeCatalogAddButton.dataset.selectedQuantity = String(addedQuantity);
                activeCatalogAddButton.setAttribute(
                    "aria-label",
                    addedQuantity > 0
                        ? `${activeCatalogProduct.name} agregado, cantidad ${addedQuantity}`
                        : `${activeCatalogProduct.name} ya alcanzó el límite permitido`
                );
            }
        }
    ...
    document.addEventListener("keydown", (event) => {
        const openDrawer = document.querySelector<HTMLElement>("[data-drawer].is-open");
        ...
        if (event.key === "Escape") {
            document.querySelectorAll<HTMLElement>("[data-drawer].is-open").forEach((drawer) => {
                closeDrawer(drawer);
            });

            document.querySelectorAll<HTMLElement>(".admin-popover.is-open").forEach((popover) => {
